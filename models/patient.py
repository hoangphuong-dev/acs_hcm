...
class MatePatient(models.Model):
    ...
    def _rec_count(self):
        rec = super(MatePatient, self)._rec_count()
    ...
    def mate_check_cancellation_flag(self):
        ...
        for rec in self:
            ...
            cancelled_appointments = self.env['mate.appointment'].sudo().search_count([
                ('date','>=', date_start), 
                ('date','<=', date_end),
                ('patient_id','=', rec.id),
                ('state', 'in', ['cancel'])
    ...
    appointment_ids = fields.One2many('mate.appointment', 'patient_id', 'Appointments')
    medical_alert_ids = fields.Many2many('mate.medical.alert', 'patient_medical_alert_rel','patient_id', 'alert_id',
    ...
    def show_weight_chart(self):
        action = self.env["ir.actions.actions"]._for_xml_id("mate_hms.action_patient_evaluation_graph_1")
    ...
    def create_invoice(self):
        ...
        invoice = self.mate_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'patient'})
    ...
    def action_evaluation(self):
        action = self.env["ir.actions.actions"]._for_xml_id("mate_hms.action_mate_patient_evaluation")
        action['domain'] = [('patient_id','=',self.id)]
        action['context'] = {'default_patient_id': self.id, 'default_physician_id': self.primary_physician_id.id}
        return action
...
class MateFamilyMember(models.Model):
    ...
    related_patient_id = fields.Many2one('mate_hms.patient', string='Family Member', help='Family Member Name', required=True)    
