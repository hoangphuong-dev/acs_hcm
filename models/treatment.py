...
class MateTreatment(models.Model):
    ...
    appointment_ids = fields.One2many('mate.appointment', 'treatment_id', string='Appointments')
    ...
    medical_alert_ids = fields.Many2many('mate.medical.alert', 'treatment_medical_alert_rel','treatment_id', 'alert_id',
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(mate_context).mate_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'treatment'})
    ...
    def action_create_procedure_invoice(self):
        ...
        invoice = self.mate_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        procedure_ids.write({'invoice_id': invoice.id})

    def view_invoice(self):
        invoices = self.invoice_id + self.patient_procedure_ids.mapped('invoice_id')
        action = self.mate_action_view_invoice(invoices)
    ...
    def mate_select_treatement_for_appointment(self):
        if self._context.get('mate_current_appointment'):
            ...
            appointment = self.env['mate.appointment'].search([('id','=',self._context.get('mate_current_appointment'))])
