...
class MateAppointmentPurpose(models.Model):
    _name = 'mate.appointment.purpose'
...
class MateAppointment(models.Model):
    ...
    patient_id = fields.Many2one('mate_hms.patient', ondelete='restrict',  string='Patient',
        required=True, index=True, help='Patient Name', tracking=1)
    image_128 = fields.Binary(related='patient_id.image_128',string='Image', readonly=True)
    physician_id = fields.Many2one('mate_hms.physician', ondelete='restrict', string='Physician', 
    ...
    evaluation_id = fields.Many2one('mate_hms.patient.evaluation', ondelete='restrict', compute=_get_evaluation,
    ...
    patient_diseases_ids = fields.One2many('mate_hms.patient.disease', readonly=True, 
    ...
    responsible_id = fields.Many2one('mate_hms.physician', "Responsible Jr. Doctor")
    medical_alert_ids = fields.Many2many('mate_hms.medical.alert', 'appointment_medical_alert_rel','appointment_id', 'alert_id',
        string='Medical Alerts', related='patient_id.medical_alert_ids')
    alert_count = fields.Integer(compute='_get_alert_count', default=0)
    consumable_line_ids = fields.One2many('mate_hms.consumable.line', 'appointment_id',
    ...
    refered_from_appointment_id = fields.Many2one("mate.appointment", string="Refered From Appointment")
    refered_from_physician_id = fields.Many2one('mate_hms.physician', related='refered_from_appointment_id.physician_id', string='Refered from Physician', tracking=1, store=True)
    ...
    refered_to_appointment_id = fields.Many2one("mate.appointment", string="Refered Appointment")
    refered_to_physician_id = fields.Many2one('mate_hms.physician', related='refered_to_appointment_id.physician_id', ondelete='restrict', string='Refered to Physician', tracking=1, store=True)
    ...
    def onchange_department(self):
        ...
        if self.department_id:
            physicians = self.env['mate_hms.physician'].search([('department_ids', 'in', self.department_id.id)])
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('name', 'New Appointment') == 'New Appointment':
                values['name'] = self.env['ir.sequence'].next_by_code('mate.appointment') or 'New Appointment'
    ...
    def action_appointment_send(self):
        ...
        ctx = {
            'default_model': 'mate.appointment',
    ...
    def action_view_treatment(self):
        ...
        if self.treatment_id:
            ...
            action['views'] = [(self.env.ref('acs_hms.view_acs_hms_treatment_appointment_tree').id, 'list')]
        return action
    ...
    def cancel_old_appointments(self):
        ...
        if self.env.user.sudo().company_id.acs_cancel_old_appointment:
            previous_appointments = self.env['mate.appointment'].search([('date','<=',yesterday),('state','in',['draft','confirm'])])
...
class StockMove(models.Model):
    ...
    appointment_id = fields.Many2one('mate.appointment', string="Appointment", ondelete="restrict")
