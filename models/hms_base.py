...
class ResUsers(models.Model):
    ...
    physician_ids = fields.One2many('mate_hms.physician', 'user_id', string='Related Physician')
    ...
    def action_create_physician(self):
        self.ensure_one()
        self.env['mate_hms.physician'].create({
            'user_id': self.id,
            'name': self.name,
        })
...
class HospitalDepartment(models.Model):
    ...
    appointment_ids = fields.One2many("mate.appointment", "department_id", "Appointments")
...
class MateConsumableLine(models.Model):
    ...
    appointment_id = fields.Many2one('mate.appointment', ondelete="cascade", string='Appointment')
