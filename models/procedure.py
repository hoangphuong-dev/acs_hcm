...
class MatePatientProcedure(models.Model):
    ...
    diseas_id = fields.Many2one('mate.diseases', 'Disease')
    description = fields.Text(string="Description")
    treatment_id = fields.Many2one('hms.treatment', 'Treatment')
    appointment_ids = fields.Many2many('mate.appointment', 'mate_appointment_procedure_rel', 'appointment_id', 'procedure_id', 'Appointments')
    ...
    consumable_line_ids = fields.One2many('mate_hms.consumable.line', 'procedure_id',
        string='Consumable Line', copy=False)
    mate_kit_id = fields.Many2one('mate.product.kit', string='Kit')
    ...
    def action_create_invoice(self):
        ...
        invoice = self.with_context(mate_context).mate_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def mate_common_invoice_procedure_data(self, invoice_id=False):
        data = []
        if self.ids:
